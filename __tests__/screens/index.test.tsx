...
describe('Home', () => {
  ...
  it('should access the login page by clicking the Access as Provider button when authenticated', () => {
    jest.clearAllMocks();
    ...
    expect(router.navigate).toHaveBeenCalledWith('(auth)/prestador/(drawer)/home');
  });

  it('should access the service catalog page by clicking on the Access as Customer button when authenticated', () => {
    jest.clearAllMocks();
    mockAuthContext.mockReturnValue({ isAuthenticated: true });

    const { getByText } = render(mockHome);
    const buttonCustomerAccess = getByText('Entrar como Cliente');
    fireEvent.press(buttonCustomerAccess);

    expect(router.navigate).toHaveBeenCalledWith('(auth)/cliente/home');
  });
});
